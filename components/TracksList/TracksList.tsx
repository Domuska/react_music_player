...
const TextOrPlayingAnimation = ({
	...
	return (
		<AnimationContainer>{displayAnimation ? <AnimatedPlayback /> : <>{text}</>}</AnimationContainer>
	);
};
...
const TrackRow = ({
	...
	return (
		<NoBorderTr onMouseEnter={onHover} onMouseLeave={onMouseLeave} $isRowActive={trackIsPlaying}>
			<TrackNumberTd $hovered={isHovered}>
				{isHovered ? (
					...
					<TextOrPlayingAnimation displayAnimation={isRowPlaybackOngoing} text={index} />
				)}
			</TrackNumberTd>
